...
var ProductsService = function () {
    ...
    var ProductsService = _classThis = /** @class */ (function () {
        function ProductsService_1() {
            this.products = [];
            var dataPath = path.join(__dirname, 'products.json');
            var jsonData = fs.readFileSync(dataPath, 'utf-8');
            this.products = JSON.parse(jsonData);
        }
        ProductsService_1.prototype.findByCategory = function (category) {
            return this.products.filter(function (p) { return p.category.includes(category); });
        };
        ProductsService_1.prototype.findByName = function (name) {
            return this.products.find(function (p) { return p.name.includes(name); });
        };
